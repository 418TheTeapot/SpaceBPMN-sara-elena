...
export default function ElementNotifications(overlays, eventBus) {
  ...
  eventBus.on(['RESET_SIMULATION_EVENT', 'SCOPE_CREATE_EVENT', 'TOGGLE_MODE_EVENT'], () => {
    this.clear();
  });
}
...
ElementNotifications.prototype.addElementNotification = function(element, options) {
  ...
  const { type, icon, text, scope = {} } = options;
  ...
  const overlayId = this._overlays.add(element, 'element-notification', {
    position,
    html: html,
    show: {
      minZoom: 0.5
  ...
  console.log('Added overlay with ID:', overlayId, 'for element:', element.id);
