...
import { restoreInitialState } from '../../../../example/modeler'; // Ensure the correct path to modeler.js
...
ToggleMode.prototype.toggleMode = async function(active = !this._active) {
  ...
  if (active) {
    ...
    // Reset the simulation and restore initial state when the mode is deactivated
    this._eventBus.fire(RESET_SIMULATION_EVENT);
    await restoreInitialState();
  }

  this._eventBus.fire(TOGGLE_MODE_EVENT, { active });
