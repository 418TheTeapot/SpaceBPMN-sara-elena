...
Animation.prototype.createAnimation = function(connection, scope, done = noop) {
  ...
  console.log('Nome del token:', tokenGfx);

  const animation = new TokenAnimation(tokenGfx, connection.waypoints, () => {
    if (isSpaceTransition(scope.element)) {
...
Animation.prototype.createStandardAnimation = function(connection, scope, done = noop) {
  const group = this._getGroup(scope);
  ...
  console.log("Connection BPMN :", connection);
...
Animation.prototype.createSpaceAnimation = function(connection, scope, done = noop) {
  ...
  const StokenGfx = this._createTokenGfx(group, scope);
  console.log('Nome del token Spaziale:', StokenGfx);

  const spaceAnimation = new TokenAnimation(StokenGfx, connection.waypoints, () => {
  ...
  console.log("Connection SPACE :", connection.length)
...
Animation.prototype._getTokenSVG = function(scope) {
  ...
  const tokenLabel = is(scope.element, 'space:Transition') ? 'Spaziale' : 'BPMN';
...
Animation.prototype._getGroup = function(scope) {
  ...
  // Mostra i token di animazione sui livelli del piano
...
function isSpaceTransition(element) {
  // Verifica se l'elemento è definito e ha un tipo
  if (element && element.type) {
    // Verifica se il tipo dell'elemento è 'space:Transition'
    return element.type === 'space:Transition';
  }
  // Se l'elemento non è definito o non ha un tipo, restituisci false
  return false;
}
