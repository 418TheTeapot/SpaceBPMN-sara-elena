...
// Crea un'animazione per una connessione
Animation.prototype.createAnimation = function(connection, scope, done = noop) {
  ...
  // Crea l'elemento grafico del token e l'animazione in base al tipo di scope
  let tokenGfx;

  if (scope.element && scope.element.type === 'space:Transition') {
    tokenGfx = this._createTokenGfxForSpace(group, scope);
  } else if (scope.element && scope.element.type.startsWith('bpmn')) {
    tokenGfx = this._createTokenGfxForBPMN(group, scope);
  } else {
    // gestione per altri tipi di elementi non supportati
    console.error('Tipo di scope non supportato:', scope.element.type);
    return;
  }
  ...
  // Nasconde l'animazione se lo scope non è mostrato
  if (!this._scopeFilter.isShown(scope)) {
    animation.hide();
  }

  // Imposta lo scope e l'elemento della connessione per l'animazione
  animation.scope = scope;
  animation.element = connection;

  // Aggiunge l'animazione all'insieme di animazioni
