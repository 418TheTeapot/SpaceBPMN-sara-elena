...
Animation.prototype.createAnimation = function(connection, scope, done = noop) {
  ...
  const group = this._getGroupScope(scope)
  ...
  console.log('Nome del token:', tokenGfx); // Spostato dopo la creazione di tokenGfx
...
Animation.prototype.createSpaceAnimation = function(connection, scope, done=noop) {
  ...
  const spaceAnimation = new TokenAnimation(StokenGfx, connection.waypoints, () => {
    ...
    done(); // Passa il parametro isSpaceToken alla funzione onAnimationComplete
  });
...
Animation.prototype._getGroupForSpace = function(scope) {
  ...
  if ('findRoot' in canvaSpace) {
    ...
    // Decidi come gestire il recupero del layer per le space animations se non è supportato da canvaSpace
  }
...
Animation.prototype._getGroupScope = function(scope) {
  if (is(scope.element, 'space:Transition')) {
    return this._getGroupForSpace(scope);
  } else {
    return this._getGroup(scope);
  }
};
