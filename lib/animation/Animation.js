...
Animation.prototype.pause = function() {
  ...
  this.each(spaceAnimation =>spaceAnimation.pause())

};

Animation.prototype.play = function() {
  this.each(animation => animation.play());
  this.each(spaceAnimation =>spaceAnimation.play())
...
Animation.prototype.createSpaceAnimation = function(connection, scope, done = noop) {
  ...
  // console.log([group, scope])
  const tokenGfx = this._createTokenGfx(group, scope);
  console.log("Token Spaziale Creato");
  // console.log(tokenGfx)
  const spaceAnimation = new SpaceTokenAnimation(tokenGfx, connection.waypoints, () => {
  ...
  spaceAnimation.setSpeed(this.getAnimationSpeed()); // Usa spaceAnimation invece di animation

  if (!this._scopeFilter.isShown(scope)) {
    spaceAnimation.hide(); // Usa spaceAnimation invece di animation
  }

  spaceAnimation.scope = scope;
  spaceAnimation.element = connection;

  this._spaceAnimations.add(spaceAnimation); // Usa spaceAnimation invece di animation

  this._eventBus.fire(ANIMATION_CREATED_EVENT, {
    spaceAnimation // Usa spaceAnimation invece di animation
  });
...
Animation.prototype._getTokenSVG = function(scope) {
  ...
  if (is(scope.element, 'space:Transition')) {
    tokenLabel = 'Spaziale';
    tokenSize = SPACE_TOKEN_SIZE; // Definire la dimensione del token spaziale
  } else {
    tokenLabel = 'BPMN';
    tokenSize = TOKEN_SIZE; // Definire la dimensione del token BPMN
  }
...
Animation.prototype._getGroup = function(scope) {
  ...
  if ('findRoot' in canvas) {
    if (is(scope.element, 'space:Transition')) {
      root = canvaSpace.findRoot(scope.element);
      layer = canvaSpace._findPlaneForRoot(root).layer;
    } else {
  ...
  // Se l'elemento è di tipo 'space:Transition', utilizza un layer diverso
  if (is(scope.element, 'space:Transition')) {
    var transitionGroup = domQuery('.bts-animation-tokens-transition', layer);
    if (!transitionGroup) {
      transitionGroup = svgCreate('<g class="bts-animation-tokens-transition" />');
      svgAppendTo(transitionGroup, layer);
    }
...
function SpaceTokenAnimation(gfx, waypoints, done) {
  TokenAnimation.call(this,gfx,waypoints,done);
}

SpaceTokenAnimation.prototype.cancelTime = function() {
  this._t = 0; // reimposta il tempo a zero
};


SpaceTokenAnimation.prototype.create = function() {
...
SpaceTokenAnimation.prototype.tick = function(tElapsed) {
  ...
  if (!part) {
    // Se non trova un segmento corrente (quindi part è undefined),
    // significa che l'animazione è completata e non facciamo nulla
    return;
  }
...
SpaceTokenAnimation.prototype.pause = function() {
  TokenAnimation.prototype.pause.call(this);
};

SpaceTokenAnimation.prototype.play = function() {
  TokenAnimation.prototype.play.call(this);
};

SpaceTokenAnimation.prototype.schedule = function() {
  TokenAnimation.prototype.schedule.call(this);
};


SpaceTokenAnimation.prototype.move = function(x, y) {
  TokenAnimation.prototype.move.call(this, x, y);
};

SpaceTokenAnimation.prototype.show = function() {
  TokenAnimation.prototype.show.call(this);
};

SpaceTokenAnimation.prototype.hide = function() {
  TokenAnimation.prototype.hide.call(this);
};

SpaceTokenAnimation.prototype.remove = function() {
  TokenAnimation.prototype.remove.call(this);
};

SpaceTokenAnimation.prototype.setSpeed = function(speed) {
  TokenAnimation.prototype.setSpeed.call(this, speed);
};
...
TokenAnimation.prototype.play = function() {
  ...
  // console.log("Stato di pausa:", this._paused); // Aggiunto questo console.log
...
TokenAnimation.prototype.tick = function(tElapsed) {
  ...
  // Se non trova un segmento corrente (quindi part è undefined),
  // significa che l'animazione è completata e chiama il metodo remove() per rimuovere l'animazione.
  if (!part) {
    // console.log("prova")
    return this.remove();
