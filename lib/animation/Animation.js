...
export default function Animation(canvas, eventBus, scopeFilter, spaceModeler) {
  ...
  eventBus.on(SCOPE_DESTROYED_EVENT, event => {
    ...
    this.clearSpaceAnimations(scope);
  });
}
...
Animation.prototype.animate = function(connection, scope, done) {
  if (is(scope.element, 'space:Transition')) {
    ...
    this.createBPMNAnimation(connection, scope, done);
  }
};
...
Animation.prototype.pauseSpace = function() {
  this.eachSpace(animation => animation.pause());
};
...
Animation.prototype.playSpace = function() {
  this.eachSpace(animation => animation.play());
};
...
Animation.prototype.eachSpace = function(fn) {
  this._spaceAnimations.forEach(fn);
};

Animation.prototype.createBPMNAnimation = function(connection, scope, done = noop) {
  const group = this._getGroup(scope);
  if (!group) {
    return;
  }

  const tokenGfx = this._createTokenGfx(group, scope, 'bpmn');
...
Animation.prototype.createSpaceAnimation = function(connection, scope, done = noop) {
  const group = this._getSpaceGroup(scope);
  if (!group) {
    return;
  }

  const tokenGfx = this._createTokenGfx(group, scope, 'space');
  const animation = new TokenAnimation(tokenGfx, connection.waypoints, () => {
    this._spaceAnimations.delete(animation);
    done();
  });
  ...
  this._spaceAnimations.add(animation);
  this._eventBus.fire(ANIMATION_CREATED_EVENT, {
    animation: animation,
    type: 'space'
  });
...
Animation.prototype.setAnimationSpeed = function(speed) {
  ...
  this.eachSpace(animation => animation.setSpeed(speed));
...
Animation.prototype.clearSpaceAnimations = function(scope) {
  this.eachSpace(animation => {
    if (!scope || animation.scope === scope) {
      animation.remove();
    }
...
Animation.prototype._createTokenGfx = function(group, scope, type) {
  const parent = svgCreate(this._getTokenSVG(scope, type).trim());
  return svgAppendTo(parent, group);
};

Animation.prototype._getTokenSVG = function(scope, type) {
  ...
  return `
    <g class="bts-token bts-token-${type}">
      <circle
        class="bts-circle"
        r="${TOKEN_SIZE / 2}"
        cx="${TOKEN_SIZE / 2}"
...
Animation.prototype._getGroup = function(scope) {
  ...
  if ('findRoot' in canvas) {
    ...
    layer = root ? canvas._findPlaneForRoot(root).layer : undefined;
  } else {
...
Animation.prototype._getSpaceGroup = function(scope) {
  var canvaSpace = this._spaceModeler._canvaspace.canvas;
  var layer;

  if ('findRoot' in canvaSpace) {
    const rootspace = canvaSpace.findRoot(scope.element);
    layer = rootspace ? canvaSpace._findPlaneForRoot(rootspace).layer : undefined;
  } else {
    layer = domQuery('.viewport', canvaSpace._svg);
  }
  ...
  var group = domQuery('.bts-space-animation-tokens', layer);

  if (!group) {
    group = svgCreate('<g class="bts-space-animation-tokens" />');
    svgAppendTo(group, layer);
  }
