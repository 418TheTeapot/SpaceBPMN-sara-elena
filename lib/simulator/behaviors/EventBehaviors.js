...
EventBehaviors.prototype.handleConditionalEvent = function(context) {
  ...
  } else if (condition === 'updateAssignment' && is(element, 'bpmn:StartEvent')) {
    console.log('ConditionalStartEventDefinition triggered: Update assignments.');
    this._activityBehavior.checkAndTriggerConditionalStartEvent(context);
  } else {
    console.log('ConditionalEventDefinition not triggered: Condition not met or not a start event.');
  }
};
