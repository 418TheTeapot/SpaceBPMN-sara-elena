...
EventBehaviors.prototype.get = function(element) {
  const behaviors = {
    ...
    'bpmn:StartEvent': this.handleConditionalStartEvent.bind(this),
...
EventBehaviors.prototype.handleConditionalStartEvent = function(context) {
  const { element, scope } = context;
  const condition = this.evaluateCondition(element);
  ...
  if (condition) {
    ...
    console.log('Condizione non soddisfatta, l\'evento di inizio non è attivato.');
  }
};