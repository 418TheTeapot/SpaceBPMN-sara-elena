...
EventBehaviors.prototype.handleStartEventConditional = function(context) {
  ...
  if (element.businessObject.eventDefinitions[0].condition === 'updateAssignment') {
    ...
    console.log("ma che elemnto prnede?", element)
    console.log('Condition:',element.businessObject.eventDefinitions[0].condition);
    //this._activityBehavior.exit(context);
    alert("Vaffanculo")
  } else {
    console.log('ConditionalStartEventDefinition not activated: sti cazzi.');
    console.log("ma che elemnto prnede?", element)

    console.log('Condition2 :',element.businessObject.eventDefinitions[0].condition);
