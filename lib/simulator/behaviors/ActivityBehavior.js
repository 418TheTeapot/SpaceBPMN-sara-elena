...
ActivityBehavior.prototype.removePlaceByName = function(placeName) {
  console.log(this._spaceModeler); // Check the value of spaceModeler
  console.log( "Questo è giusto:",this._spaceModeler._canvaspace.canvas); // Check the value of canvas
  ...
  console.log(placeToRemove); // Check the value of placeToRemove
...
ActivityBehavior.prototype.removeTransitionByName = function(transitionName) {
  const [sourcePlaceName, targetPlaceName] = transitionName.split('.');
  ...
  const canvas = this._spaceModeler._canvas;
  ...
  if (sourcePlace && targetPlace) {
    const transitionToRemove = transitions.find(transition =>
      transition.sourcePlace.name === sourcePlace.name && transition.targetPlace.name === targetPlace.name
    );

    if (transitionToRemove) {
      canvas.removeConnection(transitionToRemove);
      canvas._elementRegistry.remove(transitionToRemove);
      console.log(`Removed transition from ${sourcePlaceName} to ${targetPlaceName}`);
    } else {
      console.log(`No transition found from ${sourcePlaceName} to ${targetPlaceName}`);
    }
  } else {
    console.log(`Source or target place not found: ${sourcePlaceName}, ${targetPlaceName}`);
  }
};
ActivityBehavior.prototype.createTransitionAndPlace = function(sourcePlaceName, newPlaceName, newTransitionName) {
  const places = this._spaceModeler._places.get('Elements');
  const sourcePlace = places.find(place => is(place, 'space:Place') && place.name === sourcePlaceName);

  if (!sourcePlace) {
    console.log(`No place found with name: ${sourcePlaceName}`);
    return;
  }
  ...
  // Create new place
  const newPlace = canvas.addShape({
    id: 'place_' + new Date().getTime(), // Add an id for the new place
    type: 'space:Place',
    x: 100, // Specify the x-coordinate
    y: 100, // Specify the y-coordinate
  ...
  const newTransition = {
    id: 'transition_' + new Date().getTime(), // Add an id for the new transition
    type: 'space:Transition',
    source: sourcePlace,
    target: newPlace,
    waypoints: [
      { x: sourcePlace.x, y: sourcePlace.y },
      { x: newPlace.x, y: newPlace.y }
    ],
    name: newTransitionName
  };

  canvas.addConnection(newTransition);

  console.log(`Created new place with name: ${newPlaceName}`);
  console.log(`Created new transition with name: ${newTransitionName}`);
};
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  // this.createTransitionAndPlace("Sara", "NewPlace", "NewTransition");  //this.removePlaceByName("Sara");
  // this.removeTransitionByName("Sara-Transition");
  // AssignmentUtil.deleteTransitionByName("a.b", this._spaceModeler);
 this.removeTransitionByName("Sara-Transition");
...
// Using the utility class in ActivityBehavior
ActivityBehavior.prototype.checkAssignmentsAgainstPlaces = function(context) {
  ...
  if (element.businessObject && element.businessObject.assignment) {
    ...
    // Loop through each place in the assignment object
    Object.keys(assignmentObj).forEach(placeName => {
      // Check if the placeName matches any place name
...
ActivityBehavior.prototype.getAssignment = function(context) {
  ...
  const assignmentObj = AssignmentUtil.parseElementAssignment(element.businessObject.assignment);
  console.log('Parsed assignment:', assignmentObj); // Log the parsed assignment
  ...
  if (element && element.businessObject) {
    ...
    console.log('Assignment:', assignment); // Log the assignment
    return assignment;
  } else {
    const assignment = {
...
// Funzione generale per aggiornare le assegnazioni in base al contesto
ActivityBehavior.prototype.updateAssignments = function(context) {
  const { element } = context;
  const places = this._spaceModeler._places.get('Elements').filter(el => is(el, 'space:Place')); // Correct access

  console.log('Places in ActivityBehavior:', places);
