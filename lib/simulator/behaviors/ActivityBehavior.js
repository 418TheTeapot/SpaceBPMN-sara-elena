...
// Determina i flussi attivati
  const activatedBpmnFlows = complete
      ? element.outgoing.filter(isSequenceFlow)
      : [];

  const activatedSpaceFlows = complete
      ? element.outgoing.filter(isSequenceFlow) //isTransition non fa e non capisco perche //possiamo inziaread implementare qua la logica per "uscita" , poi cercheremo sync
      : [];

  // TRANSITION NON FA PERCHè HA UN LOGICA PER NON SO COSA , FA QUALCHE SEGMENTO E STOP
// Controllo se l'elemento ha una destinazione
  if (element.businessObject.destination) {
    const flows = this.getSpaceWeighedPath(context);
    const parentRootId = flows.length > 0 ? flows[0].sourcePlace.id : null;
    ...
    if (element.businessObject.destination === parentRootId) {
      ...
      if (flows.length > 0) {
        ...
        connections.forEach(connection => {
          // fare qlcosa con  connessioni spaziali
        });

        if (connections.length > 0) {
          const element = connections.shift();
          element['connections'] = connections;
          element['activatedFlows'] = activatedSpaceFlows;
    ...
    activatedBpmnFlows.forEach(flowElement => {
      this._simulator.enter({
        element: flowElement,
        scope: parentScope
      });
    ...
    // Gestione dei token BPMN
    if (activatedBpmnFlows.length === 0) {
      this._scopeBehavior.tryExit(parentScope, scope);
    }

    // Gestione dei token spaziali
    activatedSpaceFlows.forEach(spaceFlowElement => {
      this._simulator.enter({
        element: spaceFlowElement,
        scope: parentScope
      });
    });

    // Gestione dei token spaziali
    if (activatedSpaceFlows.length === 0) {
      this._scopeBehavior.tryExit(parentScope, scope);
    }
  }
...
//
//
//   // Altrimenti, cerca la root nel genitore
//   var place = this._spaceModeler._places.get('Elements').filter(el => is(el, 'space:Place'));
...
//     }
//   } else {
//     // Gestisce il caso in cui la root non sia definita
...
ActivityBehavior.prototype.getSpaceWeighedPath = function (context) {
  ...
  //funzione interna per calcolare percorso
  function getRoot() {
    if (element.parent && element.parent.businessObject.root) {
  ...
  for (let i = 0; i < shortestpath.length; i++) {
    for (let j = 0; j < connections.length; j++) {
      if (shortestpath[i] === connections[j].sourcePlace.id &&  shortestpath[i + 1] === connections[j].targetPlace.id) {
