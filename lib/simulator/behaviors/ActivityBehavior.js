...
ActivityBehavior.prototype.getValidityGuard = function(context) {
  ...
  console.log('Checking validity guard for element:', element.id);
  ...
  if (element.businessObject.guard.startsWith("not in")) {
    ...
    placesAdmitted = place.slice(); // Clone the place array
    console.log('Guard "not in" detected, guard array:', guardArrayNot);
    for (let i = 0; i < guardArrayNot.length; i++) {
      for (let j = 0; j < place.length; j++) {
        if (guardArrayNot[i].trim() === place[j].name.trim()) {
    ...
    console.log('Places admitted after "not in" guard check:', placesAdmitted.map(p => p.name));
    return placesAdmitted;
  } else if (element.businessObject.guard.startsWith("in")) {
    guard = element.businessObject.guard.slice(3, guardLength - 1);
    var guardArray = guard.split(",");
    console.log('Guard "in" detected, guard array:', guardArray);
    for (let i = 0; i < guardArray.length; i++) {
      for (let j = 0; j < place.length; j++) {
        if (guardArray[i].trim() === place[j].name.trim()) {
          placesAdmitted.push(place[j]);
        }
      }
    }
    console.log('Places admitted after "in" guard check:', placesAdmitted.map(p => p.name));
    return placesAdmitted;
  }
  console.log('No valid guard found, returning empty array');
  return []; // Return empty array if no valid guard found
};
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  if (eventBehavior) {
    console.log('Event behavior detected, calling event behavior');
    return eventBehavior.call(this, context);
  }

  const continueEvent = this.waitAtElement(element);
  if (continueEvent) {
    console.log('Continue event detected, signaling on event');
  ...
  if (event) {
    console.log('Message event detected, signaling on event');
    return this.signalOnEvent(context, event);
  }

  if (element.businessObject.guard) {
    console.log('Guard detected for element:', element.businessObject.guard);
    const placesAdmitted = this.getValidityGuard(context);
    let guardViolated = false;

    if (element.businessObject.guard.startsWith("not in")) {
      console.log('Checking "not in" guard condition');
      guardViolated = !placesAdmitted.some(place => place.id === element.businessObject.root);
    } else {
      console.log('Checking "in" guard condition');
      guardViolated = !placesAdmitted.some(place => place.id === element.businessObject.root);
    }
    ...
    if (guardViolated) {
      ...
      // Handle boundary event if it exists
      const boundaryFlows = element.outgoing.filter(flow => flow.isBoundaryConditionalEvent);
      if (boundaryFlows.length > 0) {
        console.log('Boundary flows detected, handling boundary event exit');
        this._handleBoundaryEventExit(context, element, boundaryFlows, scope.parent, scope);
      } else {
        console.log('No boundary flows, pausing simulation');
       // this._pauseSimulation(); // Pausing the simulation
      }
...
ActivityBehavior.prototype._handleBoundaryEventExit = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (activatedFlows.length === 0) {
    console.log('No activated flows, trying to exit scope');
    this._scopeBehavior.tryExit(parentScope, scope);
    return;
  }

  activatedFlows.forEach(flowElement => {
    console.log('Entering activated flow element:', flowElement.id);
