...
export default function ActivityBehavior(
  ...
  this._eventBus.on(RESET_SIMULATION_EVENT, () => {
    this.resetRoot();
    console.log('RESET_SIMULATION_EVENT: Root reset to initial value:', this._initialRoot);
  });

  // Register animation end event listener
  this._eventBus.on('animation.end', () => {
    this.resetRoot();
    console.log('animation.end: Root reset to initial value:', this._initialRoot);
...
ActivityBehavior.prototype.getSpaceWeighedPath = function(context) {
  ...
  // Memorizza la root iniziale solo se non è già stata memorizzata e non è null
  if (this._initialRoot === null && root !== null) {
    this._initialRoot = root;
    console.log('getSpaceWeighedPath: Initial root value saved:', this._initialRoot);
  }
...
ActivityBehavior.prototype.resetRoot = function() {
  if (this._initialRoot !== null) {
    console.log('Resetting root to:', this._initialRoot);
    // Ripristina la root per tutti gli elementi necessari
    this._elementRegistry.forEach(element => {
      if (element.parent && element.parent.businessObject) {
        element.parent.businessObject.root = this._initialRoot;
      }
    });
  } else {
    console.warn('No initial root value found to reset.');
  }
}
...
ActivityBehavior.prototype._checkConditionalStartEvents = function(placeName, key, value) {
  ...
  allElements.forEach(element => {
    if (is(element, 'bpmn:StartEvent') && element.businessObject.eventDefinitions && element.businessObject.eventDefinitions[0] === 'bpmn:ConditionalEventDefinition') {
