...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    // Attivare tutti gli eventi condizionali `StartEvent` nel registro degli elementi
    const allElements = this._elementRegistry.getAll();
    const startEventConditionals = allElements.filter(e =>
        e.type === 'bpmn:StartEvent' && e.businessObject.eventDefinitions &&
    ...
    startEventConditionals.forEach(startEventConditional => {
      ...
      // Inizializzare lo scope per il nuovo processo
      const newScope = this._simulator.createScope({
        element: startEventConditional,
        parentScope: null,
        //initiator: element.id // Aggiungiamo l'iniziatore per tracciare l'origine

      });

      console.log('New scope created:', newScope);

      // Utilizzare il simulatore per gestire l'ingresso
