...
ActivityBehavior.prototype.addPlacesAndTransition = function(assignmentObj) {
  ...
  // Aggiorna il grafo dopo aver aggiunto i nuovi nodi e archi
  this._updateGraph();
};
...
ActivityBehavior.prototype.getSpaceWeighedPath = function(context) {
  ...
  const shortestPath = g.getShortestPath(rootId, goalId);
  if (!shortestPath || shortestPath.length === 0) {
    console.error(`Il percorso più breve tra ${rootName} e ${goalName} non è stato trovato.`);
    alert(`Shortest path between ${rootName} and ${goalName} not found.`);
    return []; // Restituisci un array vuoto se il percorso non viene trovato
  }
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    // Verifica e attivazione del boundary event
    ...
    const invalidFlows = flows.filter(flow => {
      const connection = this._spaceModeler._canvaspace.canvas._elementRegistry._elements[flow.id];
      return !connection || !connection.element;
    });

    if (invalidFlows.length > 0) {
      console.warn('Invalid flows detected:', invalidFlows);
      console.warn('Destination not reachable');
      alert("Destination not reachable");

      // Verifica e attivazione del boundary event
      if (this._eventBehaviors.handleBoundaryEvents({ element, scope })) {
        console.log("Boundary event triggered due to invalid flows");
        return;
      }
...
ActivityBehavior.prototype._handleConnections = function(flows, activatedFlows, parentScope) {
  ...
  if (connections.length > 0) {
    ...
    console.warn('No connections found to handle.');
    alert("No valid connections found. Stopping the token.");
  }
};
