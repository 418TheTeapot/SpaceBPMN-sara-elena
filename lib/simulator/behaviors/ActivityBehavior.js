...
ActivityBehavior.prototype.addPlacesAndTransition = function(assignmentObj) {
  ...
  assignmentObj.add.forEach((placeName, index) => {
    ...
    if (previousPlace) {
      // Qui stai chiamando la funzione createTransition
      const transition = this.createTransition(previousPlace, place, rootElement);

      if (!transition) {
        console.error('Failed to create transition between:', previousPlace.name, 'and', place.name);
        return;
      }
    }
...
ActivityBehavior.prototype.createTransition = function(sourcePlace, targetPlace, rootElement) {
  const elementFactory = this._spaceModeler.get('olcElementFactory');
  const olcUpdater = this._spaceModeler.get('olcUpdater');
  const canvas = this._spaceModeler._canvaspace.canvas;

  console.log("CHI CAZZO è IL SOURCE:", sourcePlace);
  console.log("CHI CAZZO è IL TARGET:", targetPlace);
  ...
  let transition = elementFactory.create('connection',transitionAttrs);

  if (!transition) {
    console.error('Failed to create businessObject for transition between:', sourcePlace.name, 'and', targetPlace.name);
    return null;
  }
  ...
  transition = olcUpdater.connectionWaypoints(sourcePlace, targetPlace);
  console.log('Transition waypoints before validation:', transition);

  if (transition && transition.length > 0) {
    if (transition && transition.id) {
      canvas.addConnection(transition.id);
    } else {
      console.error('transition is not defined or does not have an id');
    }
    console.log('Created transition:', sourcePlace.name, '->', targetPlace.name);
  } else {
    console.error('Transition waypoints are invalid:', transition);
    return null;
  }
...
ActivityBehavior.prototype.processAssignments = function(context) {
  ...
  assignmentObj.add.forEach((placeName, index) => {
    ...
    if (!place) {
      console.error(`Place with name ${placeName} not found.`);
      return;
    }
