  ...
  assignmentObj.add.forEach((placeName, index) => {
    ...
    if (previousPlace) {
      ...
      const transitionBusinessObject = elementFactory.createBusinessObject('space:Transition', {
        sourcePlace: { x: sourceX, y: sourceY },
        targetPlace: { x: targetX, y: targetY }
      });
      ...
      if (!transition.businessObject) {
        console.error('Failed to create businessObject for transition between:', previousPlace.name, 'and', place.name);
        return;
      }
      ...
      transition.waypoints = olcUpdater.connectionWaypoints(previousPlace, place);
      console.log('Transition waypoints before validation:', transition.waypoints);

      // Ensure waypoints are valid numbers
      let validWaypoints = true;
      transition.waypoints.forEach(point => {
        if (isNaN(point.x) || isNaN(point.y)) {
          console.error('Invalid waypoint detected:', point);
          validWaypoints = false;
        }
      });

      if (!validWaypoints) {
        console.error('Transition has invalid waypoints and will not be added to the canvas.');
        return;
      }
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  const places = this._spaceModeler._places.get('Elements');
  const filteredPlaces = places.filter(element => is(element, 'space:Transition'));
