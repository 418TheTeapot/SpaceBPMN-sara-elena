...
ActivityBehavior.prototype.getSpaceWeighedPath = function(context) {
  ...
  const rootId = element.parent.businessObject.root || getRoot(element, places);
  const goalId = getDestination(element, places);

  // Verifica validità degli ID
  if (!rootId || !goalId) {
  //  console.error(`Invalid rootId (${rootId}) or goalId (${goalId})`);
    return [];
  }
...
//  console.log(`Nome della destinazione: ${goalName}`);
  ...
  // Log del percorso trovato
//  console.log(`Percorso trovato: ${shortestPath.join(' -> ')}`);
...
ActivityBehavior.prototype._handleDestination = function(context, element, activatedFlows, parentScope, scope) {
  const parentId = scope.parent.id;
  console.log(`Handling destination for scope.parent ID: ${parentId}`);
  ...
  if (element.businessObject.destination) {
    if (element.parent.businessObject.root) {
      ...
      // Verifica validità degli ID
      if (!rootId || !destinationId) {
        console.error(`Invalid rootId (${rootId}) or destinationId (${destinationId})`);
        return;
      }
     ...
     // console.log(`Nome della destinazione: ${destinationName}`);
      ...
      // Aggiorna la root per il successivo scope.parent
      parentScope.root = destinationId;
    //  console.log(`Root aggiornata per ${participantId} alla destinazione: ${destinationName}`);
    ...
    // Reset della root iniziale se non ci sono più destinazioni
    const participantId = element.parent.businessObject.id;
    const initialRootId = initialRoots[participantId];
    if (initialRootId) {
      parentScope.root = initialRootId;
      element.parent.businessObject.root = initialRootId;
      console.log(`Root reimpostata alla root iniziale per ${participantId}: ${getNameRoot(initialRootId, this._spaceModeler._places.get('Elements'))}`);
    }
  }
};
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (this._eventBehaviors.handleBoundaryEvents({ element, scope })) {
    return; // Boundary event handled, stop further processing
  }
  ...
  if (flows.length === 0) {
    ...
    // Non terminare la simulazione, continua con il prossimo task
    // this._simulator.exit(context);

    // Resetta la root alla root iniziale per continuare con il prossimo task
    const participantId = element.parent.businessObject.id;
    const initialRootId = initialRoots[participantId];
    if (initialRootId) {
      parentScope.root = initialRootId;
    ...
    alert("Continuing with next task/process due to unreachable destination.");
  } else {
...
ActivityBehavior.prototype.exit = function(context) {
  const { element, scope, isBoundaryEvent } = context;
  const parentScope = scope.parent;
  const complete = !scope.failed;
  const parentId = parentScope ? parentScope.id : null;

 // console.log(`exit: element.parent.businessObject.root before = ${element.parent.businessObject.root} for scope.parent ID: ${parentId}`);
  ...
  // Log per monitorare le root e i percorsi al termine della funzione exit
 // console.log(`exit: element.parent.businessObject.root after = ${element.parent.businessObject.root} for scope.parent ID: ${parentId}`);
 // console.log(`Percorso finale per ${element.parent.businessObject.id}: root = ${element.parent.businessObject.root}, destinazione = ${element.businessObject.destination}`);
};
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  // Identifica lo scope.parent corrente
  const parentScope = scope.parent;
  const parentId = parentScope.id;
  console.log('Current scope ID:', scope.id);
  console.log('Current scope.parent ID:', parentId);

  // Salva o verifica la root per il participantId
  const participantId = element.parent.businessObject.id;
  const rootId = parentScope.root || getRoot(element, this._spaceModeler._places.get('Elements'));
  console.log('Root ID:', rootId);
  console.log('Participant ID:', participantId);

  if (!initialRoots[participantId]) {
    initialRoots[participantId] = rootId;
    console.log(`Prima root salvata per ${participantId}: ${getNameRoot(rootId, this._spaceModeler._places.get('Elements'))}`);
  }

  // Assicurati che la root attuale dello scope.parent sia impostata
  if (!parentScope.root) {
    parentScope.root = rootId;
  }

  // Imposta la root corrente dell'elemento
  element.parent.businessObject.root = parentScope.root;
  ...
  if ((element.businessObject.guard !== undefined) && (element.businessObject.guard !== "")) {
    ...
    if (guard) {
      ...
      alert("Il robot è in una posizione sbagliata per eseguire il compito");

      const fullContext = { element, scope, type: 'warning' };
...
ActivityBehavior.prototype._findOrCreateScope = function(element) {
  if (this._scopeCache.has(element.id)) {
    return this._scopeCache.get(element.id);
  }

  let scope = this._simulator.findScope({ element });
  if (!scope) {
...
ActivityBehavior.prototype._findParentParticipant = function(element) {
  ...
  if (element && is(element, 'bpmn:Participant')) {
    return element;
  }
  return null;
};
...
ActivityBehavior.prototype._checkConditionalStartEvents = function(placeName, key, value) {
  console.log('Checking conditional start events for:', placeName, key, value);
  ...
  allElements.forEach(element => {
    if (is(element, 'bpmn:StartEvent') && element.businessObject.eventDefinitions) {
      ...
      if (eventDefinition.$type === 'bpmn:ConditionalEventDefinition' && eventDefinition.condition.body === assignmentCondition) {
        console.log('Matching conditional start event found:', element.id);

        const parentParticipant = this._findParentParticipant(element);
        if (parentParticipant) {
          console.log('Parent participant element found:', parentParticipant.id);

          const scope = this._findOrCreateScope(parentParticipant);
          console.log('Scope for participant:', scope);

          const eventContext = { element: element, scope: scope };
          const eventBehavior = this._eventBehaviors.get(element);
          if (eventBehavior) {
            console.log('Event behavior found for element:', element.id);
            return eventBehavior.call(this, eventContext);
          } else {
            console.warn('No behavior found for conditional start event:', element.id);
          }
        } else {
          console.warn('Parent participant element not found for element:', element.id);
        }
      }
    }
  });
