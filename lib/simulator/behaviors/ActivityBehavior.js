...
ActivityBehavior.prototype.addPlacesAndTransition = function(assignmentObj) {
  ...
  const olcRenderer = this._spaceModeler.get('olcRenderer');
  ...
  assignmentObj.add.forEach((placeName, index) => {
    ...
    if (previousPlace) {
      const connectionBusinessObject = olcElementFactory.createBusinessObject('space:Transition', { name: 'Connection' });
      ...
      // olcRenderer.drawConnection(canvas.getGraphics(connection), connection);
