...
const initialDestinations = {}; // To store initial destinations for each task
const initialRoots = {}; // To store initial roots for each participant/process
const taskPaths = {}; // To store the paths for each task
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  if (initialRootId) {
    ...
    console.warn(`No initial root found for ${participantId}`);
  }
...
ActivityBehavior.prototype.getSpaceWeighedPath = function(context) {
  ...
  if (fullPath.length > 0) {
    ...
    if (fullPath.length > 1) {
      ...
      taskPaths[element.businessObject.id] = fullPath; // Save the path for the task
    }
  } else {
...
ActivityBehavior.prototype._handleDestination = function(context, element, activatedFlows, parentScope, scope) {
  if (element.businessObject.destination) {
    if (element.parent.businessObject.root) {
      ...
      // Salva la prima destinazione incontrata per questo task
      if (!initialDestinations[taskId]) {
        initialDestinations[taskId] = destinationId;
        console.log(`Prima destinazione salvata per ${taskId}: ${destinationName}`);
      }
      ...
      if (element.businessObject.destination === element.parent.businessObject.root) {
        ...
        if (initialRootId) {
          ...
          element.businessObject.destination = initialDestinations[taskId];
        } else {
          // Handle the case where no initial root is found
          console.warn(`No initial root found for ${participantId}`);
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  const flows = taskPaths[element.businessObject.id] || this.getSpaceWeighedPath(context);
