...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    this._handleBoundaryEvents(element, context, scope);
    this._simulator.exit(context);
...
ActivityBehavior.prototype.exit = function(context) {
  ...
  let activatedFlows = complete ? element.outgoing.filter(isSequenceFlow) : [];

  // Gestisci la destinazione e ottieni l'evento di confine gestito, se presente
  const handled = this._handleDestination(context, element, activatedFlows, parentScope, scope);

  // Se l'evento di confine indica che la destinazione non è raggiungibile,
  // attiva solo il flusso di uscita associato all'evento di confine
  if (handled === 'destinationUnreachable') {
    activatedFlows = activatedFlows.filter(flow => flow === handled);
  }

  // Se non c'è una destinazione specificata o se la destinazione è vuota,
  // aggiungi il tempo di esecuzione e attiva i flussi
  ...
  // Non uscire dalla simulazione se l'evento di confine è attivato
  if (handled !== 'destinationUnreachable') {
    this._simulator.exit(context);
  }
};
