...
import {TRANSACTION} from "bpmn-js/lib/features/replace/ReplaceOptions";
...
ActivityBehavior.prototype.addPlacesAndTransition = function(assignmentObj) {
  const elementFactory = this._spaceModeler.get('olcElementFactory');
  ...
  if (!rootElement) {
    console.error('Root element not found in canvas.');
    return;
  }
  ...
  assignmentObj.add.forEach((placeName, index) => {
    ...
    if (!place) {
      const placeBusinessObject = elementFactory.createBusinessObject('space:Place', { name: placeName });
      const placeAttrs = {
        type: 'space:Place',
      ...
      place = elementFactory.createShape(placeAttrs);

      if (!place.businessObject) {
        console.error('Failed to create businessObject for place:', placeName);
        return;
      }

      place.parent = rootElement;
      console.log('Creating place:', place);

      if (!place.id || !place.businessObject) {
        console.error('Invalid place element:', place);
        return;
      }

      console.log('Adding place to canvas:', place);
      if (!canvas._elementRegistry.get(place.id)) {
        canvas.addShape(place);
      }
      olcUpdater.linkToBusinessObjectParent(place);

      // Call additional updater methods for creating a shape
      // olcUpdater.executed([{ element: place, context: { shape: place } }]);
    }

    if (previousPlace) {
      const sourceX = previousPlace.x || previousPlace.$attrs.x;
      const sourceY = previousPlace.y || previousPlace.$attrs.y;
      const targetX = place.x || place.$attrs.x;
      const targetY = place.y || place.$attrs.y;

      if (isNaN(sourceX) || isNaN(sourceY) || isNaN(targetX) || isNaN(targetY)) {
        console.error('Invalid coordinates for transition:', {
          sourceX, sourceY, targetX, targetY
        });
        return;
      }

      const transitionBusinessObject = elementFactory.createBusinessObject('space:Transition', {
        name: '10',
        sourcePlace: previousPlace.businessObject,
        targetPlace: place.businessObject
      });

      console.log('WAYPOINTS DI PREVIOUS PLACE', sourceX, sourceY);
      console.log('WAYPOINTS DI PLACE', targetX, targetY);
      ...
      console.log('Creating transition with attributes:', transitionAttrs);

      const transition = elementFactory.create('connection', transitionAttrs);

      if (!transition.businessObject) {
        console.error('Failed to create businessObject for transition between:', previousPlace.name,
            'and', place.name);
        return;
      }

      transition.parent = rootElement;
      console.log('Creating transition:', transition);

      console.log('Transition waypoints before validation:', transition.waypoints);

      if (!transition.id || !transition.businessObject) {
        console.error('Invalid transition element:', transition);
        return;
      }

      if (transition.waypoints && transition.waypoints.length > 0) {
        canvas.addConnection(transition);
        console.log('Created transition:', previousPlace.name, '->', place.name);
      } else {
        console.error('Transition waypoints are invalid:', transition.waypoints);
      }

      olcUpdater.setConnectionPlaces({ context: { connection: transition } });

      olcUpdater.linkToBusinessObjectParent(transition);
      // olcUpdater.cropConnection({ context: { connection: transition } })

      // Call additional updater methods for creating a connection
      // olcUpdater.executed([{ element: transition, context: { connection: transition } }]);
    }

    previousPlace = place;
  });

  console.log('Created places and transitions:', assignmentObj.add);
};
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  // Definisci un array di nomi di place che vuoi creare
  const placeNames = ['Place1', 'Place2']; // Puoi modificare questi nomi in base alle tue esigenze
  this.addPlacesAndTransition(placeNames);
