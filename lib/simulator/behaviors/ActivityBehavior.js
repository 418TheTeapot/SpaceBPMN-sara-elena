...
// In ActivityBehavior.js

ActivityBehavior.prototype.createTransition = function(sourcePlaceName, targetPlaceName) {
  // Get the olcElementFactory from your spaceModeler
  ...
  const sourcePlace = AssignmentUtil.findPlaceByName(sourcePlaceName, this._spaceModeler);
  const targetPlace = AssignmentUtil.findPlaceByName(targetPlaceName, this._spaceModeler);

  if (!sourcePlace || !targetPlace) {
    console.error(`Cannot find source (${sourcePlaceName}) or target (${targetPlaceName}) place.`);
    return;
  }

  // Calculate waypoints
  const waypoints = this.calculateWaypoints(sourcePlace, targetPlace);

  // Create a new space:Transition object
  const transition = olcElementFactory.createTransition(sourcePlace, targetPlace, waypoints);

  // Add the transition to the canvas
  this._spaceModeler.get('canvas').addConnection(transition);
  console.log("Canvas element registry after addition:", this._spaceModeler.get('canvas')._elementRegistry);

  if (transition) {
    console.log(`Created transition between ${sourcePlaceName} and ${targetPlaceName}`);
  } else {
    console.error(`Failed to create transition between ${sourcePlaceName} and ${targetPlaceName}`);
  }
};

// Calculate waypoints between source and target places
ActivityBehavior.prototype.calculateWaypoints = function(sourcePlace, targetPlace) {
  const sourceMid = {
    x: sourcePlace.x + sourcePlace.width / 2,
    y: sourcePlace.y + sourcePlace.height / 2
  };

  const targetMid = {
    x: targetPlace.x + targetPlace.width / 2,
    y: targetPlace.y + targetPlace.height / 2
  };

  // Simple straight line waypoints
  return [
    { x: sourceMid.x, y: sourceMid.y },
    { x: targetMid.x, y: targetMid.y }
  ];
};
