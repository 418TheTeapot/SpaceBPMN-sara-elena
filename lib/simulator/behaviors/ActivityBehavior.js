...
ActivityBehavior.prototype.getSpaceWeighedPath = function(context) {
  ...
  connections.forEach(conn => {
    if (conn.name !== undefined) {
      const weightStr = conn.name.trim(); // Rimuovi eventuali spazi bianchi
      //console.log(`Nome della connessione: ${weightStr}`); // Stampa di controllo per il debugging
      if (/^\d+$/.test(weightStr)) { // Verifica che il nome contenga solo cifre
      ...
      // } else {
      //   console.error('Il nome della connessione è undefined.');
      //   alert('A connection name is undefined. Please check your data.');
      // }
    }
  });
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  if ((element.businessObject.guard !== undefined) && (element.businessObject.guard !== "")){
    ...
    if (guard) {
      ...
      console.log('Firing GUARD_VIOLATION_EVENT with context:', { element: element, scope: scope, type: 'warning' });
      this._eventBus.fire(GUARD_VIOLATION_EVENT, { element: element, scope: scope, type: 'warning' });
  ...
  if (element.businessObject.destination) {
    if (element.parent.businessObject.root) {
      if (element.businessObject.destination === element.parent.businessObject.root) {
        ...
        //qua dobbiamo recuperare la prima root [] in assoluto,
        //se uguale allora non possiamo fare nulla
        //altrimenti possiamo fare il processo???
      } else {
