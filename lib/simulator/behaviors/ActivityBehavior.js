...
ActivityBehavior.prototype.enter = function(context) {
  ...
  if (element.businessObject.guard) {
    ...
    const guardSatisfied = this.evaluateGuard(context, placesAdmitted);

    if (!guardSatisfied) { // Modificato per controllare se la guardia non è soddisfatta
      alert("Guard condition not satisfied and it affects the flow");
      ...
      this._simulator.exit(context); // Continua il flusso normalmente se la guardia è soddisfatta
    }
  } else {
...
ActivityBehavior.prototype.evaluateGuard = function(context, placesAdmitted) {
  const { element } = context;
  const guard = element.businessObject.guard;
  const currentRoot = element.businessObject.root;

  if (!guard) return true; // Se non ci sono guardie, la condizione è soddisfatta

  if (guard.startsWith("not in")) {
    const guardPlaces = guard.slice(7, -1).split(",");
    // Verifica se la root corrente è una delle posizioni nella guardia "not in"
    return guardPlaces.includes(currentRoot);
     // La guardia è soddisfatta se la root non è nella lista "not in"
  } else if (guard.startsWith("in")) {
    const guardPlaces = guard.slice(4, -1).split(",");
    // Verifica se la root corrente è una delle posizioni nella guardia "in"
    return !guardPlaces.includes(currentRoot);
  }

  return false; // Se il formato della guardia è sconosciuto, ritorna false
};
  ...
  if (element.businessObject.destination) {
    if (element.parent.businessObject.root) {
      ...
      if (element.businessObject.destination === element.parent.businessObject.root) {
        // Imposta la root alla prima root salvata per questo partecipante
        ...
        if (initialRootId) {
          ...
          // Reimposta la destinazione per evitare loop
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    // Verifica della condizione del Boundary Event solo se il percorso non è raggiungibile
    if (this._eventBehaviors.handleBoundaryEvents({ element, scope })) {
      return; // Boundary event handled, stop further processing
    }

    // Gestione del deadlock se non ci sono percorsi alternativi e nessun boundary event attivabile
    console.warn('Deadlock detected, no alternative flows available');
    alert("Deadlock detected. No alternative flows available.");
    this._simulator.exit(context);
