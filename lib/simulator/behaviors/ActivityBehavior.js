...
export default function ActivityBehavior(
  ...
  // Aggiungi una variabile di stato per gli avvisi
  this._alertsShown = {};
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  // Assicurarsi che gli eventi condizionali abbiano la proprietà 'condition'
  if (element.businessObject.$type === 'bpmn:ConditionalEventDefinition' && !element.businessObject.condition) {
    element.businessObject.condition = 'destinationUnreachable';
    console.log('Added condition to ConditionalEventDefinition:', element);
  }
  ...
  if (element.businessObject.guard !== undefined && element.businessObject.guard !== "") {
    ...
    if (guard !== false) {
      console.log('Guard condition met, exiting context for element:', element);
      this._simulator.exit(context);
    } else {
      console.log("The robot is in a wrong position to do the task");
    }
  } else {
    console.log('No guard condition, exiting context for element:', element);
    this._simulator.exit(context);
  }
};
...
// Funzione per attivare l'evento condizionale di start
ActivityBehavior.prototype._activateConditionalStartEvent = function(eventId) {
  const startEventElement = this._elementRegistry.get(eventId);
  if (!startEventElement) {
    console.warn('StartEvent element not found:', eventId);
    return;
  }

  const startEventContext = {
    element: startEventElement,
    scope: this._simulator.createScope({
      element: startEventElement,
      parent: null,
      initiator: null
    })
  };
  this._simulator.signal(startEventContext);
};
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    // Attivare l'evento condizionale se la destinazione non è raggiungibile
    const conditionalEvent = element.attachers.find(attacher => attacher.businessObject.$type === 'bpmn:BoundaryEvent' && attacher.businessObject.eventDefinitions[0].$type === 'bpmn:ConditionalEventDefinition');
    if (conditionalEvent) {
      console.log('Found conditional event:', conditionalEvent);
      ...
      if (eventBehavior) {
        console.log('Triggering conditional event behavior');
        return eventBehavior.call(this, eventContext);
      } else {
        console.warn('No behavior found for conditional event');
      }
    } else {
      console.warn('No conditional event found');
    }

    this.simulator.exit(context); // Assicurarsi che this.simulator.exit sia definito correttamente
