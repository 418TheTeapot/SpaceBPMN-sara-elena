...
import {CONDITION_ACTIVATED_EVENT, GUARD_VIOLATION_EVENT, RESET_SIMULATION_EVENT} from "../../util/EventHelper";
...
ActivityBehavior.prototype._checkConditionalStartEvents = function(placeName, key, value) {
  ...
  allElements.forEach(element => {
    if (is(element, 'bpmn:StartEvent') &&
      ...
      if (condition === assignmentCondition && !element.businessObject.eventDefinitions[0].hasBeenTriggered) {
        ...
        const eventBehavior = this._eventBehaviors.get(element);
        if (eventBehavior) {
          // Set the flag to true after the condition has been triggered
        //  element.businessObject.eventDefinitions[0].hasBeenTriggered = true;
          ...
          this._eventBus.fire(CONDITION_ACTIVATED_EVENT, eventContext);
