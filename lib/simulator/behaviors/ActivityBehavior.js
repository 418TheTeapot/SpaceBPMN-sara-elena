...
import ConditionalStartEventBehavior from './ConditionalStartEventBehavior';
...
export default function ActivityBehavior(
  ...
  this._conditionalStartEventBehavior = new ConditionalStartEventBehavior(simulator, elementRegistry, eventBus, spaceModeler);
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  this.checkAndTriggerConditionalStartEvent(context); // Check for conditional start events
...
ActivityBehavior.prototype.processAssignments = function(context) {
  ...
  if (element._assignmentConditionChecked) {
    console.log(`Assignment condition already checked for element ${element.id}. Skipping update.`);
    return;
  }

  console.log(`Processing assignments for element ${element.id}`);
  ...
  this._assignmentUpdateCondition = true;
  console.log(`Assignment update condition set for element ${element.id}:`, this._assignmentUpdateCondition);

  if (this._conditionalStartEventBehavior.checkCondition(element, context)) {
    this._conditionalStartEventBehavior.trigger(context);
  }

  element._assignmentConditionChecked = true;
...
ActivityBehavior.prototype.checkAndTriggerConditionalStartEvent = function(context) {
  ...
  if (this._assignmentUpdateCondition) {
    ...
    startEventConditionals.forEach(startEventConditional => {
      if (startEventConditional._conditionTriggered) {
        console.log(`Conditional start event already triggered for element ${startEventConditional.id}. Skipping.`);
        return;
      }

      console.log('Found conditional start event:', startEventConditional);
      ...
      console.log('New scope created:', newScope);
...
ActivityBehavior.prototype._processDestination = function(context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    // Attivare l'evento condizionale se la destinazione non è raggiungibile e si tratta di un Boundary Event di una task
    ...
    if (conditionalEvent) {
      // Logica per attivare il Boundary Event Condizionale
    ...
    // Uscita normale dal contesto senza terminare la simulazione
    this._simulator.exit(context);
