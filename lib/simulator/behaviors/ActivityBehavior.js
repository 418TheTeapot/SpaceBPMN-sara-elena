...
ActivityBehavior.prototype.createTestPlace = function(name, x, y) {
  ...
  // Crea l'elemento shape per il place con valori predefiniti
  const placeElement = olcElementFactory.createShape({
    type: 'space:Place',
    businessObject: placeBusinessObject,
    id: placeBusinessObject.id,
    x: x || 150,        // Coordinate predefinite o passate come parametro
    y: y || 150,        // Coordinate predefinite o passate come parametro
    width: 100,         // Larghezza predefinita
    height: 100,        // Altezza predefinita
    parent: canvas.getRootElement() // Specifica l'elemento genitore
  });

  // Aggiungi il place al canvas solo se non esiste già
  if (!canvas._elementRegistry.get(placeElement.id)) {
    canvas.addShape(placeElement);
    console.log(`Created place: ${placeElement.id} (${placeElement.businessObject.name})`);
  } else {
    console.warn(`Place with id ${placeElement.id} already exists in the canvas.`);
  }

  return placeElement; // Ritorna l'elemento creato
};


ActivityBehavior.prototype.createTransition = function(sourcePlace, targetPlace) {
  // Ottieni l'element factory dal spaceModeler
  const olcElementFactory = this._spaceModeler.get('olcElementFactory');
  const canvas = this._spaceModeler._canvaspace.canvas;

  // Crea il business object per la transition
  const transitionBusinessObject = olcElementFactory.createBusinessObject('space:Transition', {
    name: 'Test Transition'
  });

  // Crea l'elemento shape per la transition con valori predefiniti
  const transitionElement = olcElementFactory.createShape({
    type: 'space:Transition',
    businessObject: transitionBusinessObject,
    id: transitionBusinessObject.id,
    source: sourcePlace,     // Elemento sorgente
    target: targetPlace,     // Elemento destinazione
    waypoints: [
      { x: sourcePlace.x + sourcePlace.width / 2, y: sourcePlace.y + sourcePlace.height / 2 },
      { x: targetPlace.x + targetPlace.width / 2, y: targetPlace.y + targetPlace.height / 2 }
    ]
  });

  // Aggiungi la transition al canvas solo se non esiste già
  if (!canvas._elementRegistry.get(transitionElement.id)) {
    canvas.addConnection(transitionElement);
    console.log(`Created transition from ${sourcePlace.businessObject.name} to ${targetPlace.businessObject.name}`);
  } else {
    console.warn(`Transition with id ${transitionElement.id} already exists in the canvas.`);
  }
};// Esempio di utilizzo delle funzioni createTestPlace e createTransition

ActivityBehavior.prototype.createTestTransition = function() {
  // Crea due elementi di tipo space:Place
  const sourcePlace = this.createTestPlace('Test Place 1', 150, 150);
  const targetPlace = this.createTestPlace('Test Place 2', 350, 150);

  // Aggiungi entrambi gli elementi al canvas
  this._spaceModeler._canvaspace.canvas.addShape(sourcePlace);
  this._spaceModeler._canvaspace.canvas.addShape(targetPlace);

  // Crea una transizione tra i due elementi
  this.createTransition(sourcePlace, targetPlace);
};
...
ActivityBehavior.prototype.processAssignments = function(context) {
  ...
  if (assignmentObj["add"]) {
    ...
    // Trova gli elementi usando i nomi
    const sourcePlace = this._spaceModeler._canvaspace.canvas._elementRegistry.get(sourcePlaceName);
    const targetPlace = this._spaceModeler._canvaspace.canvas._elementRegistry.get(targetPlaceName);

    if (sourcePlace && targetPlace) {
      // Usa createTransition per creare la transizione
      this.createTransition(sourcePlace, targetPlace);
    } else {
      console.warn(`Places not found: ${sourcePlaceName} or ${targetPlaceName}`);
    }
  }
...
ActivityBehavior.prototype.enter = function(context) {
  ...
  // Recupera gli elementi dal registro
  const sourcePlaceReg = this._spaceModeler._canvaspace.canvas._elementRegistry.get(sourcePlace.id);
  const targetPlaceReg = this._spaceModeler._canvaspace.canvas._elementRegistry.get(targetPlace.id);

  if (sourcePlaceReg && targetPlaceReg) {
    this.createTransition(sourcePlaceReg, targetPlaceReg);
  } else {
    console.error('Failed to retrieve the places from the registry');
  }
