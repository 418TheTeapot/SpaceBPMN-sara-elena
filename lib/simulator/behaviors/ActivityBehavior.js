...
export default function ActivityBehavior(
  ...
  this._conditionalStartTriggered = false;
...
ActivityBehavior.prototype._updatePlaceAssignments = function(assignmentObj, assignmentModified, element) {
  console.log('Starting _updatePlaceAssignments'); // Initial log

  const places = this._spaceModeler._places.get('Elements');
  const placeNames = places.map(place => place.name);
  //console.log('Place names:', placeNames); // Log place names

  let firstUpdateAssignmentTriggered = false;

  Object.keys(assignmentObj).forEach(placeName => {
    //console.log('Checking place:', placeName); // Log current place name

    if (placeNames.includes(placeName)) {
      const placeAssignments = assignmentObj[placeName];
      //console.log('Place assignments for', placeName, ':', placeAssignments); // Log assignments for the place

      Object.keys(placeAssignments).forEach(key => {
        const alertKey = `${placeName}-${key}`;
        //console.log('Checking alertKey:', alertKey); // Log current alert key
        ...
        if (!this._alertsShown[alertKey]) {
          ...
          // Check for the first update and set condition
          if (!firstUpdateAssignmentTriggered) {
            console.log('First update assignment triggered'); // Log first update trigger
            firstUpdateAssignmentTriggered = true;

          }

          // Handle conditional start events after setting the condition
          const conditionalStartEvents = this.findScopeOfConditionalStartEvent();
          console.log('Conditional start events:', conditionalStartEvents); // Log conditional start events
          conditionalStartEvents.forEach(event => {
            console.log('Event:', event); // Log event

            const startEventConditional = this._elementRegistry.get(event.element.id);
            console.log('Start Event Conditional:', startEventConditional); // Log start event conditional

            if (startEventConditional) {
              console.log('Updating startEventConditional condition to updateAssignment'); // Log condition update
              //startEventConditional.businessObject.eventDefinitions.condition = 'updateAssignment';
              startEventConditional.businessObject.eventDefinitions[0].condition = 'updateAssignment';
              console.log("che cazzo prende:",startEventConditional.businessObject)
              console.log("Vediamo che condizione ha messo:", startEventConditional.businessObject.eventDefinitions[0].condition )

              const eventContext = {
                element: startEventConditional,
                scope: event.parentScope
              };

              console.log('Event context:', eventContext); // Log event context
              alert('Triggering conditional event for updateAssignment');
              // Trigger the conditional start event
              // this.triggerConditionalStartEvent(eventContext);
              console.log('Triggering conditional start event for element:', startEventConditional);
              console.log("elemnteo del cazzo:",startEventConditional)
              // Find and trigger the conditional start event behavior
              ...
              console.log('Start event conditional not found in element registry');
            }
          });
        } else {
...
ActivityBehavior.prototype.findScopeOfConditionalStartEvent = function() {
  console.log('Finding scope of conditional start events'); // Initial log
  const elements = this._elementRegistry.getAll();
  console.log('All elements:', elements); // Log all elements
  ...
  console.log('Conditional start events found:', conditionalStartEvents); // Log found conditional start events

  return conditionalStartEvents.map(event => {
    const condition = this.getConditionalStartEventCondition(event);
    console.log('Event condition:', event.businessObject.eventDefinitions[0].condition); // Log event condition
...
ActivityBehavior.prototype.getConditionalStartEventCondition = function(element) {
  ...
  console.log('Conditional Event Definition:', conditionalEventDefinition); // Log conditional event definition
  ...
  if (element.businessObject.eventDefinitions[0].condition && element.businessObject.eventDefinitions[0].condition) {
    console.log('Condition:', element.businessObject.eventDefinitions[0].condition); // Log condition
    return element.businessObject.eventDefinitions[0].condition;
  } else {
    console.log('Condition is undefined'); // Log undefined condition
    return null;
  }
};
...
ActivityBehavior.prototype.processAssignments = function(context) {
  ...
  this._updatePlaceAssignments(assignmentObj, element);
};
