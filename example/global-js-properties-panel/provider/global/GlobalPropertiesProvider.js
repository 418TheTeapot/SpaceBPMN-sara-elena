...
function PlaceGroup(element, injector) {
  ...
  const entries = [
    // Definisci qui le proprietà specifiche per gli elementi Place
    ...NameProps({ element }),
    ...IdProps({ element }),
  ];
...
function TransitionGroup(element, injector) {
  const translate = injector.get('translate');

  const entries = [
    ...NameProps({ element }),
    ...IdProps({ element }),
    // Definisci qui le proprietà specifiche per gli elementi Transition
  ];
...
function getGroups(element, injector) {
  ...
  const groups = [
    ...
    PlaceGroup(element, injector), // Aggiungo il gruppo per gli elementi Place
    TransitionGroup(element, injector) // Aggiungo il gruppo per gli elementi Transition
  ];
...
export default class GlobalPropertiesProvider {
...
GlobalPropertiesProvider.$inject = [ 'propertiesPanel', 'injector' ];
