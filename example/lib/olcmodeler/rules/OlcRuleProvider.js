...
OlcRuleProvider.prototype.init = function () {
  ...
  this.addRule('connection.create', function (context) {
    ...
    var existingConnections = self._elementRegistry.filter(function(element) {
      return is(element, 'space:Transition') && element.source === source && element.target === target;
    });
    //TODO this leads to reverse connections being created because of diagram-js' Connect.js trying to

    return is(source, 'space:Place') && is(target, 'space:Place') && existingConnections.length === 0 && { type: 'space:Transition' };
  });

  this.addRule('connection.start', function (context) {
    var source = context.source;
    return is(source, 'space:Place') && { type: 'space:Transition' };
  });
