...
export default function SpacePropertiesProvider(propertiesPanel, translate, eventBus, spaceModeler) {
    ...
    this.getGroups = function (element) {
        return function (groups) {
            ...
            if (is(element, 'bpmn:StartEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
                groups.push(createDetailsMessage(element, translate));
            }
            if (is(element, 'bpmn:IntermediateThrowEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
                groups.push(createDetailsMessage(element, translate));
            }
            if (is(element, 'bpmn:IntermediateCatchEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
                groups.push(createDetailsMessage(element, translate));
            }
    ...
    function createDetailsMessage(element, translate) {
        ...
        if (is(element, 'bpmn:StartEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
            label = translate('Start Event Message');
            description = translate('Details of Start Event Message.');
        } else if (is(element, 'bpmn:IntermediateCatchEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
            label = translate('Read the SMS received');
            description = translate('Read the received message.');
        } else if (is(element, 'bpmn:IntermediateThrowEvent') && hasEventDefinition(element, 'bpmn:MessageEventDefinition')) {
            label = translate('Send a Message');
            description = translate('Write the message if you want to send.');
        } else {
    ...
    function hasEventDefinition(element, eventDefinition) {
        return element.businessObject.eventDefinitions && element.businessObject.eventDefinitions.some(definition => is(definition, eventDefinition));
    }
}
